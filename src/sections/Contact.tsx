        ...
        <motion.form variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} transition={{ delay: 0.1 }} onSubmit={onSubmit} noValidate className="rounded-brand border border-line bg-white/[0.04] p-6 shadow-card backdrop-blur sm:p-8">
          <div className="grid gap-5 sm:grid-cols-2">
            <label className="grid gap-2 text-sm font-medium text-zinc-200">
              Name
              <input name="name" className="field" aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? 'name-error' : undefined} />
              {errors.name && <span id="name-error" className="text-xs text-red-300">{errors.name}</span>}
            </label>
            <label className="grid gap-2 text-sm font-medium text-zinc-200">
              Email
              <input name="email" type="email" className="field" aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? 'email-error' : undefined} />
          ...
          <label className="mt-5 grid gap-2 text-sm font-medium text-zinc-200">
            Message
            <textarea name="message" rows={6} className="field resize-none" aria-invalid={Boolean(errors.message)} aria-describedby={errors.message ? 'message-error' : undefined} />
