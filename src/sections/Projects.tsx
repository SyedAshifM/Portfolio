...
export const Projects = () => {
  ...
  return (
    <Section id="projects" eyebrow="Projects" title="Selected work shaped for real business workflows.">
      <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid gap-6 lg:grid-cols-3">
        {portfolio.projects.map((project) => (
          <motion.article key={project.title} variants={fadeUp} whileHover={{ y: -10 }} className="group overflow-hidden rounded-brand border border-line bg-panel shadow-card">
            <button type="button" onClick={() => setSelectedProject(project)} className="relative block aspect-[16/10] w-full overflow-hidden text-left" aria-label={`Open details for ${project.title}`}>
